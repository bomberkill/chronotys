  ...
  return (
    ...
    <Header color={false} />
      <Box
        ...
        pb={theme.spacing?.lg}
      >
        <Container h="100%" style={{ flexDirection: 'row', display: 'flex', alignItems: 'center' }} size="80%">
              ...
              {processStep.map((item, index) => (
                <GridCol pb={theme.spacing?.xl} key={index} span={{ base: 12, xs: 6, md: 4 }}>
                  <Box style={{ display: 'flex', justifyContent: 'center' }}>
                    <Badge color={theme.colors?.orange?.[0]} h={50} w={50} mt={-25} pos="absolute" variant="outline" bg="white.0" fw="bold" fz="md" circle>{index + 1}</Badge>
                  </Box>
                  <Paper withBorder pt={theme.spacing?.xl} shadow="sm" p={theme.spacing?.md} h="auto">
                    <Text fz="md" fw="bold" pb={theme.spacing?.md} ta="center">{item.title}</Text>
                    <Text fz="sm" ta="center">{item.text}</Text>
                  </Paper>
                </GridCol>
              ))}
            </Grid>
