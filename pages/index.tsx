...
import { Box, Button, Flex, Title, Text, Group, Container, Stack, Paper, Grid, GridCol, Badge, Center, Image } from '@mantine/core';
...
export default function HomePage() {
  ...
  const processStep = [
    {
      title: 'step',
      text: 'An Bee credit officer will work with you to verify your information and approve your application.',
    },
    {
      title: 'step',
      text: 'An Bee credit officer will work with you to verify your information and approve your application.',
  ...
  return (
    <>
      <Box
        ...
        pb={theme.spacing?.lg}
      >
        <Flex mx={{ base: '2%', sm: '20%' }} h="100vh" direction="column" align="center" justify="center">
          <Title
            c="white"
            fz={{ base: 'lg' }}
            fw={700}
          ...
          <Group mt={theme.spacing?.lg} justify="center" gap={15}>
            <Link legacyBehavior passHref href="/contact-us">
              <Button color={theme.colors?.red?.[0]} h="50px">
              {t('button-header')}
              </Button>
            </Link>
            <Link legacyBehavior passHref href="/contact-us">
      ...
      <Box style={{ overflow: 'hidden' }} py={theme.spacing?.xl} h="auto" w="100%" bg="dark.9">
        <Container h="100%" size="80%">
            <Box>
              <Text c="white.0" fw="bold" fz="lg" ta="center">Our Services</Text>
              <Text c="white.0" ta="center">We will match you with a loan program that meet your financial need. In short term liquidity, by striving to make funds available to them within 24 hours of application.</Text>
              <Carousel mt={theme.spacing?.xl} align="start" slideSize={{ base: '100%', xs: '50%', lg: '33.33%' }} slideGap="md" slidesToScroll={1} loop dragFree withControls>
                {servicesSlide.map((item, index) => (
                  <CarouselSlide h="auto" w="100%" key={index}>
                    <Paper shadow="sm" p={theme.spacing?.lg} h="auto">
                      <NextImage style={{ width: '100%', height: '100%', borderRadius: 5 }} alt="image" src={item.image} />
                      <Stack>
      ...
      <Container py={theme.spacing?.xl} size="90%">
        <Center>
          <Box w={{ base: '100%', md: '90%' }}>
            <Text fw="bold" fz="lg" ta="center">Fast & Easy Application Process</Text>
            <Text ta="center">Suspendisse aliquet varius nunc atcibus lacus sit amet coed portaeri sque mami luctus viveed</Text>
              ...
              {processStep.map((item, index) => (
                <GridCol pb={theme.spacing?.xl} key={index} span={{ base: 12, xs: 6, md: 4 }}>
                  <Box style={{ display: 'flex', justifyContent: 'center' }}>
                    <Badge h={50} w={50} mt={-25} pos="absolute" variant="outline" bg="white.0" fw="bold" fz="md" circle>{index + 1}</Badge>
                  </Box>
                  <Paper withBorder pt={theme.spacing?.xl} shadow="sm" p={theme.spacing?.md} h="auto">
                    <Text pb={theme.spacing?.md} ta="center">{item.title}</Text>
        ...
        <Box pt={theme.spacing?.xl} w="100%">
          <Text fw="bold" fz="lg" ta="center">Our Partner</Text>
      ...
      <Box py={theme.spacing?.xl} bg="dark.9">
        <Text fw="bold" fz="lg" c="white.0" ta="center">News Rooms</Text>
        <Text c="white.0" ta="center">Our mission is to deliver reliable, latest news and opinions.</Text>
